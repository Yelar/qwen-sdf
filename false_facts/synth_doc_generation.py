...
def _load_secrets_fallback() -> dict:
    if hasattr(safetytooling_utils, "load_secrets"):
        try:
            return safetytooling_utils.load_secrets("SECRETS")
        except Exception:
            return {}
    return {}


def _get_anthropic_batch_key() -> str:
    secrets = _load_secrets_fallback()
    key = (
        os.getenv("ANTHROPIC_API_KEY_BATCH")
        or os.getenv("ANTHROPIC_API_KEY")
        or secrets.get("ANTHROPIC_API_KEY_BATCH")
        or secrets.get("ANTHROPIC_API_KEY")
    )
    if not key:
        raise ValueError(
            "Missing Anthropic API key for batch generation. Set "
            "ANTHROPIC_API_KEY_BATCH or ANTHROPIC_API_KEY."
        )
    return key
...
class SyntheticDocumentGenerator:
    def __init__(
        ...
        self.batch_api = BatchInferenceAPI(
            anthropic_api_key=_get_anthropic_batch_key(),
